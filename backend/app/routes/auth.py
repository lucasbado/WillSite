...
def listar_clientes_admin():
    ...
    usuarios = (
        db.session.query(User, func.count(WorkOrder.id).label("total_os"))
        .outerjoin(WorkOrder)
        .group_by(User.id)
        .all()
    )
...
def detalhes_cliente_admin(user_id):
    ...
    ordens = (
        WorkOrder.query.filter_by(client_id=user_id)
...
def verificar_cpf(cpf):
    ...
    if user:
        return (
            jsonify({"existe": True, "msg": "Este CPF já possui um cadastro ativo."}),
            200,
        )
...
# --- ROTA: REGISTRAR USUÁRIO PELO ADMIN ---
...
@auth_bp.route(
    "/admin/usuarios/<int:user_id>", methods=["DELETE"], strict_slashes=False
)
@jwt_required()
def deletar_usuario_admin(user_id):
    ...
    try:
        ...
        db.session.rollback()
        return (
            jsonify({"msg": "Erro: Este usuário possui ordens de serviço vinculadas."}),
            400,
        )
...
def forgot_password():
    ...
    if not user:
        ...
        return (
            jsonify(
                {
                    "msg": "Se este e-mail estiver cadastrado, um link de recuperação será enviado."
                }
            ),
            200,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "msg": "Erro ao enviar e-mail de recuperação. Tente novamente mais tarde."
                }
            ),
            500,
        )
...
def reset_password():
    ...
    try:
        ...
        db.session.commit()
        return (
            jsonify({"msg": "Senha atualizada com sucesso! Faça login novamente."}),
            200,
        )
    except Exception as e:
        db.session.rollback()
...
def verify_email():
    ...
    try:
        ...
        db.session.commit()
        return (
            jsonify({"msg": "E-mail verificado com sucesso! Sua conta está ativa."}),
            200,
        )
    except Exception as e:
        db.session.rollback()
